...
Required env vars (read via `requireEnv`, which throws if missing): `DISCORD_TOKEN`, `DISCORD_CLIENT_ID`, `SUPABASE_JWT_SECRET`. The HTTP API adds optional `API_PORT` (default 3000) and `API_CORS_ORIGINS` (default `*`). Music adds optional `LAVALINK_HOST` (default `lavalink`), `LAVALINK_PORT` (default 2333), `LAVALINK_PASSWORD` (default `youshallnotpass`), `LAVALINK_SECURE` (default `false`).
